...
export default function CTASection({ onRegisterClick }: CTASectionProps) {
  return (
    <section className="py-12 px-6 md:px-12 max-w-7xl mx-auto w-full">
      <div className="bg-utility rounded-3xl overflow-hidden grid md:grid-cols-2 items-center shadow-xs border border-brand/25">
        
        {/* Left column text */}
        <div className="p-8 md:p-16 flex flex-col gap-6 items-start text-left">
          <div className="p-3 bg-app-surface rounded-2xl">
            <Car className="h-8 w-8 text-brand" />
          </div>
          
          <h2 className="text-2xl md:text-4xl font-extrabold text-content leading-tight">
            Bạn muốn cho thuê xe?
          </h2>
          
          <p className="text-sm text-content-secondary leading-relaxed max-w-md">
          ...
          <div className="flex flex-wrap gap-4 mt-2">
            <Link
              href="/about"
              className="border border-content text-content hover:bg-content hover:text-night-content font-bold py-2.5 px-6 rounded-lg text-sm transition cursor-pointer"
            >
              Tìm hiểu ngay
            </Link>
            <button 
              onClick={onRegisterClick}
              className="bg-brand hover:bg-brand-hover text-on-brand font-bold py-2.5 px-6 rounded-lg text-sm transition cursor-pointer shadow-md hover:scale-105 active:scale-95 duration-200"
            >
              Đăng ký xe
            </button>
          </div>
