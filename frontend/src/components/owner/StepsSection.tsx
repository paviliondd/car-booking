...
export default function StepsSection() {
  const steps = [
    {
      id: 1,
      icon: <FileText className="h-8 w-8 text-[#008F5A]" />,
    ...
    {
      id: 2,
      icon: <UserCheck className="h-8 w-8 text-[#008F5A]" />,
    ...
    {
      id: 3,
      icon: <Car className="h-8 w-8 text-[#008F5A]" />,
      title: 'Duyệt xe',
  ...
  return (
    <section className="py-16 px-6 md:px-12 max-w-7xl mx-auto flex flex-col gap-10">
      <h2 className="text-2xl md:text-3xl font-extrabold text-center text-gray-950 dark:text-white">
        3 bước đăng ký xe trên <span className="text-[#008F5A]">datxe</span>
      </h2>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {steps.map((step) => (
          <div 
            key={step.id} 
            className="relative bg-emerald-50 border border-emerald-100 rounded-2xl p-8 flex flex-col gap-4 shadow-xs transition hover:shadow-md hover:border-[#008F5A]/20 duration-300"
          >
            {/* Absolute badge number top-right */}
            <div className="absolute top-4 right-4 bg-[#008F5A] text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">
              {step.id}
            </div>
