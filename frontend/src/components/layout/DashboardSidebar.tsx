  ...
  return (
    <aside className={`z-30 flex min-h-dvh shrink-0 flex-col border-r border-slate-800 bg-slate-950 text-slate-300 transition-[width] duration-200 ${isMinimized ? 'w-[76px]' : 'w-[248px]'}`}>
      <div className="flex h-[68px] items-center justify-between border-b border-slate-800 px-4">
        {!isMinimized && <Link href="/dashboard" onClick={onNavigate} className="flex min-h-11 items-center gap-2 rounded-xl font-bold text-white"><span className="rounded-lg bg-emerald-600 px-2 py-1 text-sm">datxe</span><span className="text-xs tracking-wider text-slate-400">ADMIN</span></Link>}
        <button type="button" onClick={() => setIsMinimized((value) => !value)} className="flex min-h-11 min-w-11 cursor-pointer items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-800 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400" aria-label={isMinimized ? 'Mở rộng menu' : 'Thu gọn menu'}>
      ...
      <nav className="flex flex-1 flex-col gap-1.5 overflow-y-auto p-3" aria-label="Điều hướng quản trị">
        ...
        {navigation.filter((item) => !item.adminOnly || user.role === 'ADMIN').map(({ href, label, icon: Icon }) => {
          ...
          return <Link key={href} href={href} onClick={onNavigate} aria-current={active ? 'page' : undefined} title={isMinimized ? label : undefined} className={`flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${active ? 'bg-emerald-500/15 text-emerald-300 ring-1 ring-emerald-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><Icon className="h-5 w-5 shrink-0" />{!isMinimized && <span>{label}</span>}</Link>;
        })}
      </nav>
      <div className="border-t border-slate-800 p-3"><Link href="/" onClick={onNavigate} title={isMinimized ? 'Về website' : undefined} className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-slate-400 transition hover:bg-slate-800 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"><ExternalLink className="h-5 w-5 shrink-0" />{!isMinimized && <span>Về website</span>}</Link></div>
    </aside>
  );
}
