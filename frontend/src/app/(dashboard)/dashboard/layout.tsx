    ...
    if (typeof window !== 'undefined') {
      ...
      try {
        ...
        if (me.role !== 'OWNER' && me.role !== 'ADMIN' && me.role !== 'STAFF') {
          toast.error('Tài khoản của bạn không có quyền chủ xe');
          router.push('/');
          return;
        }
        setLoading(false);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#14142b] flex flex-col items-center justify-center text-white select-none gap-3">
        <Loader2 className="h-10 w-10 text-[#00B14F] animate-spin" />
