...
export default function ContractPage() {
  ...
  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    ctx.lineWidth = 2.5;
    ctx.lineCap = 'round';
    ctx.strokeStyle = '#f3f4f6'; // Netural light white stroke on dark board
      ...
      {bookingData && contractData && (
        <div className="grid md:grid-cols-3 gap-8 relative z-10">
          ...
          <div className="md:col-span-2 flex flex-col gap-6">
            <div className="glass-panel p-6 rounded-xl border border-white/5 h-[480px] overflow-y-auto flex flex-col gap-4">
              <h3 className="font-extrabold text-white text-lg border-b border-white/5 pb-3">Nội Dung Hợp Đồng Thuê Xe</h3>
          ...
          <div className="md:col-span-1 flex flex-col gap-6">
            <div className="glass-panel p-6 rounded-xl border border-purple-500/20 flex flex-col gap-4">
              ...
              <div className="border border-white/10 rounded-lg overflow-hidden bg-gray-950/80">
                <canvas 
                  ref={canvasRef}
                  width={220}
                  height={150}
              ...
              <div className="flex gap-2">
                <button 
                  onClick={clearCanvas}
                  className="flex-1 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-semibold py-2 rounded-lg cursor-pointer transition border border-white/5"
                >
                  Xóa vẽ lại
                </button>
              </div>
              ...
              {contractData.renterSignature ? (
                <div className="bg-green-500/10 border border-green-500/20 text-green-400 p-4 rounded-xl text-center flex flex-col items-center gap-2">
                ...
                <button 
                  ...
                  className="w-full gradient-btn text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 cursor-pointer text-sm disabled:opacity-50"
                >
